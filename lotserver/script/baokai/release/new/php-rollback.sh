...
FILE=$(basename $1)
echo ""
echo -ne "确定要回滚【PHP】吗？y), n)  \t: "
read ask
if [ $ask == "y" -o $ask == "Y" ];then
	if [ -f "/data/backup/rollback/frontend/$FILE" ]; then
		cd /usr/local/nginx/html
		rm -rf `ls|grep -v interface|grep -v moblieAdmin` && rm -rf .svn/
		tar zxf /data/backup/rollback/frontend/$FILE -C /usr/local/nginx/html
